...
import AutomationCodeGenerator from "./AutomationCodeGenerator";
...
function explainStatus(status: number) {
  const explanations: Record<number, { title: string; message: string }> = {
    200: { title: "200 OK", message: "The API understood the request and returned the requested data." },
    201: { title: "201 Created", message: "The API successfully created a new resource. Save the returned ID for later requests." },
    204: { title: "204 No Content", message: "The request succeeded, but there is intentionally no JSON response body." },
    400: { title: "400 Bad Request", message: "The request is incomplete or invalid. Check required fields, values, and JSON syntax." },
    401: { title: "401 Unauthorized", message: "A valid Bearer token is missing or expired. Generate a new access token and try again." },
    403: { title: "403 Forbidden", message: "The token is valid, but it does not have permission for this operation." },
    404: { title: "404 Not Found", message: "The requested customer, account, or endpoint does not exist. Check the URL and saved ID." },
    405: { title: "405 Method Not Allowed", message: "The endpoint exists, but it does not accept this HTTP method. Check the Allow response header." },
    409: { title: "409 Conflict", message: "The request conflicts with the current data—for example, deleting a funded account." },
    415: { title: "415 Unsupported Media Type", message: "The server cannot read this body format. Send valid JSON with Content-Type: application/json." },
    422: { title: "422 Unprocessable Entity", message: "The JSON is valid, but a business rule failed—for example, insufficient funds." },
    429: { title: "429 Too Many Requests", message: "The rate limit was reached. Wait for the Retry-After time shown in the response headers." },
    500: { title: "500 Internal Server Error", message: "The server encountered an unexpected problem. Record the request ID and try again later." },
    503: { title: "503 Service Unavailable", message: "The service is temporarily unavailable. Wait briefly, then retry the request." },
  };
  return explanations[status] ?? { title: `${status} Response`, message: "Read the response message and headers to understand what the API returned." };
}
...
export default function ApiLabClient() {
  ...
  const visibleOperations = useMemo(() => {
    ...
    return scenarios.map((item, index) => ({ item, index })).filter(({ item }) => !query || [item.group, item.label, item.method, item.description, item.path(ids)].join(" ").toLowerCase().includes(query));
  }, [operationQuery, ids]);
  ...
  async function sendRequest(event: FormEvent) {
    ...
    try {
      ...
      setInspector({ method: scenario.method, url: displayUrl, requestHeaders, requestBody, responseHeaders: Object.fromEntries(response.headers.entries()) });
  ...
  async function runNegativeTest(kind: NegativeKind) {
    ...
    try {
      ...
      const cases: Record<NegativeKind, { method: string; url: string; body?: string; token: string; label: string; contentType?: string }> = {
        ...
        "403": { method: "GET", url: `${baseUrl}/api/v1/status/403`, token: accessToken, label: "Forbidden scope" },
        "405": { method: "PUT", url: `${baseUrl}/api/v1/status/405`, token: accessToken, label: "Wrong HTTP method" },
        "415": { method: "POST", url: `${baseUrl}/api/v1/status/415`, body: "firstName=Betappa", token: accessToken, label: "Unsupported media type", contentType: "text/plain" },
        "429": { method: "GET", url: `${baseUrl}/api/v1/status/429`, token: accessToken, label: "Rate limit reached" },
        "503": { method: "GET", url: `${baseUrl}/api/v1/status/503`, token: accessToken, label: "Service unavailable" },
      }[kind];
      const requestId = `req_${crypto.randomUUID().slice(0, 8)}`;
      const requestHeaders: Record<string, string> = { "Content-Type": cases.contentType ?? "application/json", Accept: "application/json", "X-Request-ID": requestId };
  ...
  function downloadResponse() {
    if (!result) return;
    const blob = new Blob([JSON.stringify(formatResponseForDisplay(result.data), null, 2)], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `api-response-${result.status || "input"}.json`;
    document.body.appendChild(link);
    link.click();
    link.remove();
    URL.revokeObjectURL(url);
    setCopied("Response downloaded");
  }

  return <main className={`mode-${mode}`}><LearningToolsStyles /><a className="whats-new-fab" href="/whats-new"><span>New</span> What&apos;s New</a><a className="feedback-fab" href="/feedback">Feedback</a>
    ...
    <section className="career-toolkit"><div><p className="eyebrow">New learning tools</p><h2>Go beyond basic API requests</h2><p>Open a focused workspace when you are ready. The main practice page stays simple while advanced SDET tools remain one click away.</p></div><div className="career-toolkit-grid"><article><span>⚡</span><div><b>SDET Practice Lab</b><p>Simulate defects and run CSV or JSON data-driven API tests.</p><a href="/sdet-lab">Open advanced lab →</a></div></article><article><span>?</span><div><b>Interview Preparation</b><p>Practise fundamentals, automation, scenarios, and project explanations.</p><a href="/interview-prep">Start interview prep →</a></div></article><article><span>▶</span><div><b>2-Minute Project Demo</b><p>A captioned walkthrough for recruiters, educators, and new learners.</p><a href="/demo">Play guided demo →</a></div></article></div></section>
    <QuickNavigation />
